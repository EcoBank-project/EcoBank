 ...
 * @Service
 * 
 * @RequiredArgsConstructor public class CustomOAuth2UserService implements
 * OAuth2UserService<OAuth2UserRequest, OAuth2User>{ private final
 * UserRepository userRepository; private final HttpSession httpSession;
 * 
 ...
 * OAuth2User> delegate = new DefaultOAuth2UserService(); OAuth2User oAuth2User
 ...
 * 
 * User user = saveOrUpeate(attributes);
 * 
 * httpSession.setAttribute("user", new SessionUser(user));
 * 
 * return new DefaultOAuth2User( Collections.singleton(new
 * SimpleGrantedAuthority(user.getResp())), attributes.getAttributes(),
